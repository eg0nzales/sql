...
ClientData AS (
    -- From Brand-to-Market relationships with proper joins and columns
    SELECT 
...
WHERE 
    ...
    AND COALESCE(cd.market_dsc, '') NOT LIKE 'xx%' 
    AND COALESCE(cd.market_dsc, '') NOT LIKE 'zz%' 
    AND NOT (
        cd.client_nm IS NULL 
        AND cd.market_dsc NOT LIKE '%-%'
    )
ORDER BY 
    CASE WHEN cd.client_nm IS NULL THEN 1 ELSE 0 END,  -- unassigned last
    cd.client_nm,
    cd.market_dsc;
